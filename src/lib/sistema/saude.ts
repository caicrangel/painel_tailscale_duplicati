...
export function diagnosticarSaude(e: EntradaSaude): DiagnosticoSaude {
  ...
  if (!e.banco.ok) {
    ...
    if (
      e.banco.usoConexoes !== null &&
      e.banco.usoConexoes >= USO_CONEXOES_ALTO
    ) {
      problemas.push({
        nivel: "atencao",
  ...
  if (e.ciclos.length === 0) {
    problemas.push({
      nivel: "falha",
      texto:
        "O worker nunca registrou um ciclo — o monitoramento não está rodando.",
    });
  } else {
    ...
    if (parados.length > 0 && parados.length === e.ciclos.length) {
      problemas.push({
        nivel: "falha",
        texto:
          "Nenhum ciclo do worker rodou recentemente — o container do worker parece parado.",
      });
    } else {
      for (const c of parados) {
        problemas.push({
          nivel: "atencao",
          texto: `O ciclo ${c.kind} parou de rodar.`,
        });
      }
    }
    for (const c of e.ciclos) {
      if (!c.ok)
        problemas.push({
          nivel: "falha",
          texto: `O último ciclo ${c.kind} falhou.`,
        });
    }
    for (const kind of esperados) {
      if (!porKind.has(kind)) {
        problemas.push({
          nivel: "atencao",
          texto: `O ciclo ${kind} ainda não rodou nenhuma vez.`,
        });
      }
    }
  }
  ...
  if (!e.tailscaleConfigurado) {
    problemas.push({
      nivel: "atencao",
      texto:
        "Tailscale sem credencial: o status das máquinas não é atualizado.",
    });
  }
  ...
  if (e.notificacoesFalhas > 0) {
    problemas.push({
      nivel: "atencao",
      texto: `${e.notificacoesFalhas} notificação(ões) não foram entregues nos últimos 7 dias.`,
    });
  }
